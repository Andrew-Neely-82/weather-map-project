(() => {
  ...
  $.get("https://api.openweathermap.org/data/2.5/weather?lat=57&lon=-2.151", {
    ...
    .then(
      $.get("https://api.openweathermap.org/data/2.5/forecast?lat=57&lon=-2.151", {
        APPID: weatherKey,
        q: "Fort Bragg, US",
        units: "imperial",
      }).done(function (data) {
        ...
        function getDay(day) {
          ...
          let high = dayData.reduce((acc, item) => {
            return item.main.temp_max > acc ? item.main.temp_max : acc;
          }, 0);
          let low = dayData.reduce((acc, item) => {
            return item.main.temp_min < acc ? item.main.temp_min : acc;
          }, 100);
          let humidity = dayData.reduce((acc, item) => {
            return item.main.humidity > acc ? item.main.humidity : acc;
          }, 0);
          let wind = dayData.reduce((acc, item) => {
            return item.wind.speed > acc ? item.wind.speed : acc;
          }, 0);
          return {
            high: high,
            low: low,
        ...
        let monday = getDay("2022-12-02");
        let tuesday = getDay("2022-12-03");
        let wednesday = getDay("2022-12-04");
        let thursday = getDay("2022-12-05");
        let friday = getDay("2022-12-06");
        $("#container-2-1").html(`
          <div class="card">
            <h3>Mon</h3>
